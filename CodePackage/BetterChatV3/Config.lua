...
return {
	Permissions = {
		Ranks = { 
			...
			[1] = "Player", --> Rank ID 1 will automatically be assigned to each user (the config below can determine their final permission level)
			[2] = "Executive",
			[3] = "Staff",
			[4] = "Dev",
			[5] = "Manager",
			[6] = "Founder",
			[10] = "Owner" --> Automatically assigned to the group creator or game creator
		},
		Users = {
	...
	ChatTags = {
		MaxAmountTags = 1, --> Maximum amount of tags one user can have
		Users = {
			...
			["GoodGuyNick"] = { --> Assign Roblox a red 'Owner' tag
				{
					Text = "Heavy Hitter DMCA Kingpin",
					Color = Color3.fromRGB(255,0,0),
					Priority = 100
				}
			}
		...
		Ranks = {
			[2] = { --> Assign the owner of the game with the chat a yellow owner tag
				{	
					Text = "Executive",
					Color = Color3.fromRGB(141, 21, 21),
			...
			[3] = { --> Assign the owner of the game with the chat a yellow owner tag
				{	
					Text = "Staff",
			...
			[4] = { --> Assign the owner of the game with the chat a yellow owner tag
				{	
					Text = "Dev",
					Color = Color3.fromRGB(219, 0, 4),
					Priority = 9 --> If you have a max amount of tags and one user goes over it, the tags with the lowest priority will be removed first.
				}
			},
			[5] = { --> Assign the owner of the game with the chat a yellow owner tag
				{	
					Text = "Manager",
					Color = Color3.fromRGB(154, 12, 181),
					Priority = 10 --> If you have a max amount of tags and one user goes over it, the tags with the lowest priority will be removed first.
				}
			},
			[6] = { --> Assign the owner of the game with the chat a yellow owner tag
				{	
					Text = "Founder",
					Color = Color3.fromRGB(31, 207, 0),
					Priority = 11 --> If you have a max amount of tags and one user goes over it, the tags with the lowest priority will be removed first.
				}
			},
			[10] = { --> Assign the owner of the game with the chat a yellow owner tag
				{	
					Text = "Founder",
					Color = Color3.fromRGB(100, 0, 0),
					Priority = 12 --> If you have a max amount of tags and one user goes over it, the tags with the lowest priority will be removed first.
				}
			},
		}
	},
	TextColors = {
		Ranks = {
			[10] = Color3.fromRGB(255,255,0) --> Assign the owner role a golden text color
		},
		Users = {
	...
	UI = {
		...
		Fonts = {
			Chatbar = Enum.Font.Merriweather, --> Chatbar font
			TextFont = Enum.Font.Merriweather --> Every other text's font in the chat
		},
		Colors = {
		...
		ChannelBarEnabled = false, --> Channel navigation bar at the top of the chat
	...
	Messages = {
		...
		IncludeIcon = false,	--> Display a player thumbnail next to their username in the chat
		UseViewportForIcon = true, --> Use a viewport frame for this? (Helpful for Roleplay games where your avatar isn't the same as it normally would be)
		MaximumLength = 300, --> Maximum allowed message length
	...
	BubbleChat = {
		...
		Config = {
			TypingIndicator = false, --> Some dots to indicate you're typing
			EasingStyle = Enum.EasingStyle.Quad, --> The chat bubble's tween style
			Length = 0.2, --> The length per tween in the UI
			MaxMessages = 4, --> The max amount of bubble chat messages above one user's head at once
			FadeoutTime = 15, --> Message fading time
			TextSize = 16, --> This is the chat bubble's text size.
			Padding = 8, --> Padding in offset on each side of the message.
			BubbleFont = Enum.Font.Merriweather, --> The chat bubble's primary font.
			...
			Roundness = 2, --> The chat bubble's roundness in pixels
		}
	},
	SettingsMenu = { --> Settings for the chat's interface
		Enabled = false, --> Do you want it to be enabled?
